...
Amazon Linux 2023 arm64 `t4g.micro`, a no-ingress security group, Session
Manager access, and an instance role limited to the one runtime secret and
write-only recovery objects. A value-blind bootstrap installs Docker and the
tracked production assets; systemd stops the stack before materializing a
validated secret under `/run/jobcron`, so Jobcron and Caddy fail closed. The
trusted Mac performs registry-token delivery, RDS administration, recovery
copying, and private-path user verification through Session Manager.
...
- The replacement instance role may use standard Session Manager permissions,
  `secretsmanager:GetSecretValue` for the one runtime secret, and write-only S3
  access for `jobcron/*`; it may not read the RDS master secret, mutate the
  runtime secret, list/read/delete recovery objects, or administer RDS.
- The host has no EC2 key pair, no SSH, no inbound rule, required IMDSv2, an
  encrypted 8 GiB `gp3` root volume, and no instance-metadata access from
  containers.
...
- `scripts/jobcron_runtime_test.go` — fake-AWS/Docker/PostgreSQL shell contract
  tests.
- `infra/terraform/production/compute.tf` — AL2023 discovery, host networking,
  EC2, instance profile, and least-privilege IAM.
...
- `infra/terraform/production/outputs.tf` — add sensitive/private operational
  selectors without values in documentation.
...
output.runtime_secret_arn      sensitive = true
output.recovery_bucket_arn     sensitive = true
output.recovery_bucket_name    sensitive = true
```
...
access, no secret version, no destroy/replace action, and no old-resource
change. A missing address, renamed interface, stale checkpoint, or non-clean
post-apply plan stops Slice 4; do not infer or substitute an address.

## Slice 4 Terraform Action Allow-List

The reviewed saved plan may contain exactly these create actions:

```text
aws_security_group.replacement_host                                  create
aws_vpc_security_group_egress_rule.replacement_host_ipv4             create
aws_vpc_security_group_ingress_rule.database_from_replacement_host   create
...
- Consumes: a manually supplied full `release_sha` that exists in this
  repository.
- Produces: private `ghcr.io` package `jobcron`, a tag formed from `sha-` plus
  the first 12 lowercase hexadecimal characters of `release_sha`, immutable
  digest, and private `image.json` evidence.
...
6. build `deploy/production/Dockerfile` with Buildx for only `linux/arm64`;
7. push `ghcr.io/${GITHUB_REPOSITORY_OWNER,,}/jobcron:sha-${release_sha:0:12}`;
8. capture the Buildx digest without adding it to tracked files;
9. query the owner package API and fail unless visibility is exactly `private`;
10. write only commit, platform, visibility, and digest to the private workflow
    summary available to repository operators.
...
- Consumes:
  `JOBCRON_RUNTIME_SECRET_ID`, `/run/jobcron/registry-token`, the tracked
  Compose/Caddy files, and standard AWS/Docker/PostgreSQL CLIs.
...
extracts the exact approved fields with `jq -e`, rejects wrong JSON types, and
writes through a new temporary directory before atomic rename. It never uses
`set -x`, `env`, `printenv`, or value-bearing diagnostics.
...
- `key_name` is null and the host security group has no ingress;
- the only database ingress is TCP `5432` from the replacement-host security
  group;
...
- outputs are sensitive and do not expose endpoint, bucket, ARN, or IP in a
...
`/opt/jobcron` with root ownership, enables Docker and the recovery timer, and
does not start Jobcron. The tracked systemd environment uses the stable logical
secret name `jobcron-production-runtime`; IAM still scopes access to
`aws_secretsmanager_secret.runtime.arn`. User data contains no registry token,
secret ARN, runtime value, endpoint, image digest, or private identifier.
...
Start from one accepted synthetic plan and prove rejection of:

- any address outside the eight-entry allow-list;
- update, delete, replace, import, move, or forget;
- EIP association, Cloudflare, DNS, ingress, key pair, port `22`, RDS, secret
  version, or old-resource action;
...
the exact allow-list, the two aggregate ceilings, required categories,
24-hour freshness, and a clean Slice 3 checkpoint. It never prints plan
fragments, addresses containing resolved private selectors, or cost-source
URLs.
...
only `linux/arm64`, and the digest matches the pushed manifest. Save the result
to private `image.json`.

- [ ] **Step 3: Preserve rollback image evidence**

If a prior approved digest exists, retain it. Do not delete package versions.
The new digest becomes `current`; the prior digest becomes `previous`.

Stop on a mutable-only tag, public/internal package, multi-platform mismatch,
missing digest, or unexpected workflow permission.
...
Confirm verified objects are eligible for expiry only after 14 days and
unverified objects remain for 90 days. Do not enable Object Lock and do not
delete any recovery object in Slice 4.
...
- The saved plan differs from the eight exact create actions.
